...
echo "$input--->$response, Content Length: $content_length" >> output.txt
else
echo "$input--->$response, Content Length not available" >> output.txt
fi
done
done
if [ "$response" == "403" ]; then
...
content_length=$(curl -s -I $target/$input | grep -i 'content-length' | awk '{print $2}')
if [ -n "$content_length" ]; then
echo "$input--->$response, Content Length: $content_length"
else
echo "$input--->$response, Content Length not available"
fi
sleep $delay
done
done
